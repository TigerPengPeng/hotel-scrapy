...
def square_distance(pointA, pointB):
    ...
    dimensions = 2 # assumes both points have the same dimensions
...
class KDTree():
    ...
    def __init__(self, data):
        def build_kdtree(point_list, depth):
            ...
            axis = depth % 2 # assumes all points have the same dimension
    ...
    def query(self, query_point, t=1):
        ...
        return result
class kdtree:
    def __init__(self,hotelList):
        self.hotelList=hotelList
        self.create(self.hotelList)
    def create(self,hotelList):
        dataList=[]
        for i in range(len(hotelList)):
            dataList.append((hotelList[i].lat,hotelList[i].lon,i))
        self.tree = KDTree.construct_from_data(dataList)

    def query(self,hotel,r):
        if self.tree:
            reList=[]
            for tu in self.tree.query(query_point=(hotel.lat,hotel.lon), t=r):
                reList.append(self.hotelList[tu[-1]])
            return reList
        else:
            return []
...
if __name__=="__main__":
    data = [(1.0,2.0,"A"),(4.0,0.0,"B"),(-5.0,2.0,"C"),(-10.0,5.0,"D"),(9.0,8.0,"D"),(4.0,2.0,"E"),(-5.0,2.0,"F")]
    tree = KDTree.construct_from_data(data)
    nearest = tree.query(query_point=(-5.0,4.0,"T"), t=3)
    print nearest
