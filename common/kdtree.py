...
class Node(object):
    ...
    def get_data_store(self):
        """ Returns the information of node stored
        self.data store dimensions and node information and information store at pos data[-1] """
        return self.data[-1]
...
def check_dimensionality(point_list, dimensions=None):
    ...
    for each element in tuple.
        ...
        tuple[0...-2] store node's dimensions. """
    dimensions = dimensions or len(point_list[0]) - 1
    for p in point_list:
        if len(p) - 1 != dimensions:
