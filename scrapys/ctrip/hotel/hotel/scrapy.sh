#!/bin/bash
# Program:
#      Function: This Program starts scrapy programs named ctrip_city and ctrip_hotel. And when scrapy ctrip_city is done,
#                then scrapy ctrip_hotel begins to run 
# History:
...
# $2 info_log 
# $3 console_log
function ctrip_city_and_ctrip_hotel()
{
    nohup scrapy crawl ctrip_city -a info_log=$2 >> $3 2>&1 \
    && nohup scrapy crawl ctrip_hotel -a cityid=$1 -a info_log=$2 >> $3 2>&1
    if [ $? -ne 0 ];then
        return 1
    else
        return 0
...
ctrip_city_and_ctrip_hotel ${cityid} ${info_log} ${console_log}
if [ $? -ne 0 ];then
